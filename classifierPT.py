...
class Classifier:
    ...
    def normalizeColumn(self, columnNumber):
       ...
       nkl = [k[1][columnNumber] for k in self.data]
       median = self.getMedian(nkl)
       sig = self.getAbsoluteStandardDeviation(nkl, median)
       self.medianAndDeviation.append((median, sig))
       for k in self.data:
           k[1][columnNumber] = (k[1][columnNumber] - median) / sig
       pass
    
    def normalizeVector(self, v):
        vec = list(v)
        for i in range(len(vec)):
            (median, sig) = self.medianAndDeviation[i]
            vec[i] = (vec[i] - median) / sig
        return vec
